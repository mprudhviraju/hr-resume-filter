...
import { popLoginMessage, useAuth } from '../contexts/AuthContext';

export default function LoginPage() {
  const { login } = useAuth();
  const [error, setError] = useState<string>('');
  const [loading, setLoading] = useState(false);
  const [infoMessage] = useState<string>(() => popLoginMessage() || '');
    ...
    <div className="min-h-screen flex items-center justify-center px-4" style={{ backgroundColor: 'var(--bg-subtle)' }}>
      <div className="w-full max-w-sm">
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-6">
          ...
          {infoMessage && (
            <div className="mb-5 bg-blue-50 border border-blue-200 text-blue-700 px-3 py-2.5 rounded-lg text-xs flex items-start gap-2">
